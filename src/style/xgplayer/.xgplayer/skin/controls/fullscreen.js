...
let s_fullscreen = function () {
    ...
    if (fullscreenBtn.type === "img") {
        btn = createImgBtn(
            "fullscreen",
            fullscreenBtn.url.request,
            fullscreenBtn.width,
            fullscreenBtn.height
        )
    } else {
        btn = createDom(
            "xg-fullscreen",
    ...
    let tips = createDom(
        "xg-tips",
        `<span class="xgplayer-tip-requestfull">${tipsText.requestfull}</span>
                                        <span class="xgplayer-tip-exitfull">${tipsText.exitfull}</span>`,
        {},
        "xgplayer-tips"
    )
