...
let s_definition = function () {
    ...
    function onCanplayResourceReady() {
        ...
        list.forEach(item => {
            ...
            if (player.dash) {
                tmp.push(
                    `<li url='${item.url}' cname='${item.name}' class='${
            ...
            function (e) {
                ...
                if (li && li.tagName.toLocaleLowerCase() === "li") {
                    ...
                    (li.tagName.toLocaleLowerCase() === "p" ||
                        li.tagName.toLocaleLowerCase() === "em")
                ) {
                    if (sniffer.device === "mobile") {
    ...
    player.switchDefinition = function (definitionObj = {}) {
        ...
        for (let i = 0; i < liList.length; i++) {
            if (
                liList[i].getAttribute("cname") === definitionObj.name ||
                liList[i].getAttribute("url") === definitionObj.url ||
                i === definitionObj.index
            ) {
                liList[i].click()
            }
        }
