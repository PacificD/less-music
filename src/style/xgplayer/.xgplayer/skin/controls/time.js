...
let s_time = function () {
    ...
    let container = createDom(
        "xg-time",
        `<span class="xgplayer-time-current">${player.currentTime || format(0)}</span>
                                           <span>${player.duration || format(0)}</span>`,
        {},
        "xgplayer-time"
    )
    player.once("ready", () => {
    ...
    let onTimeChange = function () {
        ...
        if (player.videoConfig.mediaType !== "audio" || !player.isProgressMoving || !player.dash) {
            container.innerHTML =
                `<span class="xgplayer-time-current">${format(player.currentTime || 0)}</span>` +
