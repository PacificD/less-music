...
function renderList(root, textTrack, offText, isDefaultShow) {
    ...
    textTrack.forEach(item => {
        tmp.push(
            `<li data-id=${item.id} data-language=${item.language} class="${
                item.isDefault && isDefaultShow ? "selected" : ""
            }">${item.label}</li>`
        )
    })
...
let s_textTrack = function () {
    ...
    const container = createDom(
        "xg-texttrack",
        `<ul></ul><p class="name">${controlText}</p>`,
        { tabindex: 7 },
        "xgplayer-texttrack"
    )
    ...
    const onListUpdate = function (data) {
        ...
        data.list.length > 0
            ...
            : removeClass(player.root, "xgplayer-is-texttrack")
