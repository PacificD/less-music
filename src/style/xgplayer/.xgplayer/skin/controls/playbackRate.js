...
let s_playbackRate = function () {
    ...
    list.forEach(item => {
        if (
            player.config.defaultPlaybackRate &&
            player.config.defaultPlaybackRate.toString() === item.name
        ) {
            item.selected = true
        ...
        tmp.push(
            `<li cname='${item.name}' class='${item.selected ? "selected" : ""}'>${item.rate}</li>`
        )
    })
        ...
        container.addEventListener(
            ...
            e => {
                ...
                if (li && li.tagName.toLocaleLowerCase() === "li") {
                    ...
                    player.config.playbackRateActive === "click" &&
                    li &&
                    (li.tagName.toLocaleLowerCase() === "p" ||
                        li.tagName.toLocaleLowerCase() === "span")
                ) {
                    if (sniffer.device === "mobile") {
    ...
    player.switchPlaybackRate = function (playbackRateObj = {}) {
        ...
        for (let i = 0; i < liList.length; i++) {
            if (
                !hasClass(liList[i], "selected") &&
                (liList[i].getAttribute("cname") === `${playbackRateObj.playbackRate}` ||
                    i === playbackRateObj.index)
            ) {
                liList[i].click()
            }
        }
