...
let s_progress = function () {
        ...
        if (player.config.progressDot && typeOf(player.config.progressDot) === "Array") {
            player.config.progressDot.forEach(item => {
                if (item.time >= 0 && item.time <= player.duration) {
                    ...
                    if (item.duration >= 0) {
                        dot.style.width =
                            (Math.min(item.duration, player.duration - item.time) /
                                player.duration) *
                                100 +
                            "%"
                    }
    ...
    player.addProgressDot = function (time, text, duration, style) {
        ...
        if (time >= 0 && time <= player.duration) {
            let dot = createDom(
                "xg-progress-dot",
                text ? `<span class="xgplayer-progress-tip">${text}</span>` : "",
                {},
                "xgplayer-progress-dot"
            )
            ...
            if (duration >= 0) {
                dot.style.width =
                    (Math.min(duration, player.duration - time) / player.duration) * 100 + "%"
            }
            if (style) {
                for (let k in style) {
                    dot.style[k] = style[k]
    ...
    let onLoadedmetadata = function () {
        if (player.config.thumbnail) {
            if (player.config.thumbnail.isShowCoverPreview && !coverPreviewContainer) {
                ...
                coverPreviewContainer = createDom(
                    "xg-coverpreview",
            ...
            <xg-point class="xgplayer-coverpreview-point"></xg-point>
                ...
                coverPreviewOuter = coverPreviewContainer.querySelector(
                    ".xgplayer-coverpreview-outer"
                )
                coverPreviewPoint = coverPreviewContainer.querySelector(
    ...
    if (
        typeof player.config.disableSwipeHandler === "function" &&
        typeof player.config.enableSwipeHandler === "function"
    ) {
    ...
    events.forEach(item => {
        container.addEventListener(item, function (e) {
            ...
            let move = function (e) {
                ...
                if (now < 0) now = 0
                if (
                    player.config.allowSeekPlayed &&
                    Number(now).toFixed(1) > player.maxPlayedTime
                ) {
                } else {
                    progress.style.width = `${(w * 100) / containerWidth}%`

                    if (
                        player.videoConfig.mediaType === "video" &&
                        !player.dash &&
                        !player.config.closeMoveSeek
                    ) {
                        ...
                        if (time) {
                            time.innerHTML = `<span class="xgplayer-time-current">${format(
                                now || 0
                            )}</span><span>${format(player.duration)}</span>`
                        }
                    }
                }

                if (player.config.thumbnail && player.config.thumbnail.isShowCoverPreview) {
                    coverPreviewPoint.innerHTML = `<span>${format(now)}</span> / ${format(
                        player.duration || 0
                    )}`
                    ...
                    thumbnail.style.backgroundImage = `url(${
                        tnailUrls[Math.ceil((index + 1) / (tnailCol * tnailRow)) - 1]
                    })`
                    let indexInPage =
                        index +
                        1 -
                        tnailCol * tnailRow * (Math.ceil((index + 1) / (tnailCol * tnailRow)) - 1)
                    let tnaiRowIndex = Math.ceil(indexInPage / tnailRow) - 1
                    let tnaiColIndex = indexInPage - tnaiRowIndex * tnailRow - 1
                    thumbnail.style["background-position"] = `-${tnaiColIndex * tnailWidth}px -${
                        tnaiRowIndex * tnailHeight
                    }px`
            ...
            let up = function (e) {
                ...
                if (
                    ...
                    (player.videoConfig && player.videoConfig.mediaType === "audio") ||
                    player.dash ||
                    player.config.closeMoveSeek
                ) {
                    let w = (isRotate ? e.clientY : e.clientX) - left
    ...
    container.addEventListener(
        ...
        function (e) {
            ...
            const isRotate = isRotateFullscreen(player)
            let containerLeft = isRotate
                ? container.getBoundingClientRect().top
                : container.getBoundingClientRect().left
            let containerWidth = isRotate
                ? container.getBoundingClientRect().height
                : container.getBoundingClientRect().width

            let compute = function (e) {
                let now =
                    (((isRotate ? e.clientY : e.clientX) - containerLeft) / containerWidth) *
                    player.duration
                now = now < 0 ? 0 : now
    ...
    let onTimeupdate = function () {
        ...
        if (!player.isProgressMoving && !player.isSeeking && !player.seeking) {
            ...
            const prevPrecent =
                Number(progress.style.width.replace("%", "") || "0") /
                Number(container.style.width || "100")
    ...
    let onCacheUpdate = function () {
        ...
        if (buffered && buffered.length > 0) {
            ...
            for (let i = 0, len = buffered.length; i < len; i++) {
                if (
                    player.currentTime >= buffered.start(i) &&
                    player.currentTime <= buffered.end(i)
                ) {
                    end = buffered.end(i)
